...
interface ProcessedBet {
  ...
  // These might be null for bets without valid CLV
    ...
    >
      ...
      >
        ...
        <div className="p-6">
                    ...
                    <td className="py-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium 
                        ${bet.status === 'won' ? 'bg-green-100 text-green-800' : 
                          ...
                          bet.status === 'void' ? 'bg-purple-100 text-purple-800' :
          ...
          {/* Expected value analysis - only show if CLV data exists */}
          {bet.clv !== null && (
            <div className="mb-8">
          ...
          {/* Note for bets without CLV */}
          {bet.clv === null && (
            <div className="mb-8 p-4 bg-yellow-50 rounded-lg">
              <h3 className="text-lg font-semibold mb-2 text-yellow-800">No Closing Line Data</h3>
              <p className="text-yellow-700">
                This bet doesn't have closing line value (CLV) data available, so expected value analysis cannot be performed.
              </p>
            </div>
          )}
        </div>
...
// ===== Main Component =====
export default function AllBetsHistoryPage() {
  // ===== State variables =====
  const [filteredBets, setFilteredBets] = useState<ProcessedBet[]>([]);
  const [startDate, setStartDate] = useState<string>('');
  const [endDate, setEndDate] = useState<string>('');
  const [includePendingBets, setIncludePendingBets] = useState<boolean>(true);
  const [includePlayerProps, setIncludePlayerProps] = useState<boolean>(true);
  ...
  const fetchDataWithDates = (start: string, end: string) => {
    ...
    axios.get(`${API_URL}/api/all-bets-history?${params.toString()}`)
      .then(response => {
        if (!response.data) {
        ...
        const { bets, stats, sportsbook_stats, sport_stats, status_stats } = response.data;
        ...
        if (stats) {
          setStats({
            ...
            push_bets: stats.push_bets || 0,
            void_bets: stats.void_bets || 0,
        ...
        const formattedStatusStats = (status_stats || []).map((status: any) => ({
          name: status.status || "",
          betCount: status.bet_count || 0,
          totalStake: status.total_stake || 0,
          totalProfit: status.total_profit || 0,
          expectedProfit: status.expected_profit || 0,
          roi: status.roi || 0,
          expectedRoi: status.expected_roi || 0,
          avgEV: status.avg_ev || 0
        }));
      ...
      .catch(err => {
        console.error("Error fetching all bets history data:", err);
  ...
  const fetchAllBetsData = async () => {
    console.log("Fetching all bets history data with filters:", { 
      includePendingBets, 
      includePlayerProps, 
      startDate, 
      endDate 
    ...
    try {
      const response = await axios.get(`${API_URL}/api/all-bets-history?${params.toString()}`);
      console.log("[fetchAllBetsData] received response:", response);
  ...
  // Loading state
  if (isLoading) {
    return <LoadingSpinner size="large" message="Loading all bets history data..." />;
  }
  ...
  if (!stats) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-bold mb-6">All Bets History</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">All Bets History</h1>
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-500">
            Analyzing {stats.total_bets} total bets ({stats.valid_clv_bets} with valid CLV)
          </span>
          <button
            onClick={() => window.location.reload()}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Adjust filters to refine your betting history analysis</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="mb-6">
      ...
      {/* Key Metrics Section */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-gray-800">Overall Performance Metrics</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <Card className="bg-white">
            <CardContent className="pt-6">
              ...
              <p className="text-xs text-gray-500 mt-2">
                {stats.winning_bets} of {stats.winning_bets + stats.losing_bets} settled bets
              </p>
            </CardContent>
          </Card>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-2xl font-bold mt-1 text-gray-700">
                    {stats.avg_odds > 0 ? `+${Math.round(stats.avg_odds)}` : Math.round(stats.avg_odds)}
                  </p>
                </div>
                <div className="text-xl">🎲</div>
              </div>
              <p className="text-xs text-gray-500 mt-2">
                Avg stake: {formatMoney(stats.avg_stake)}
              </p>
            </CardContent>
          </Card>
      ...
      {/* Bet Status Summary */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-gray-800">Bet Status Summary</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <Card className="bg-green-50">
            <CardContent className="pt-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-green-600">{stats.winning_bets}</p>
          ...
          <Card className="bg-red-50">
            <CardContent className="pt-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-red-600">{stats.losing_bets}</p>
          ...
          <Card className="bg-yellow-50">
            <CardContent className="pt-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-yellow-600">{stats.pending_bets}</p>
          ...
          <Card className="bg-gray-50">
            <CardContent className="pt-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-600">{stats.push_bets}</p>
                <p className="text-sm text-gray-700">Push</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-purple-50">
            <CardContent className="pt-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-purple-600">{stats.void_bets}</p>
                <p className="text-sm text-purple-700">Void</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-blue-50">
            <CardContent className="pt-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">{stats.total_bets}</p>
      ...
      <div>
        ...
        <Tabs defaultValue="sportsbook">
          ...
          <TabsContent value="status" className="bg-white rounded-lg shadow overflow-hidden">
            <div className="overflow-x-auto">
              <table className="data-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {statusStats.map((status) => (
                    <tr key={status.name}>
                      <td>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium 
                          ${status.name === 'won' ? 'bg-green-100 text-green-800' : 
                            status.name === 'lost' ? 'bg-red-100 text-red-800' : 
                            status.name === 'pending' ? 'bg-yellow-100 text-yellow-800' : 
                            status.name === 'push' ? 'bg-gray-100 text-gray-800' :
                            status.name === 'void' ? 'bg-purple-100 text-purple-800' :
                            'bg-gray-100 text-gray-800'}`
                        }>
                          {status.name}
                        </span>
                      </td>
                      <td>{status.betCount}</td>
                      <td className="text-left">{formatMoney(status.totalStake)}</td>
                      <td className={`text-left ${getColorClass(status.totalProfit)}`}>
                        {formatMoney(status.totalProfit)}
                      </td>
                      <td className="text-left">{formatMoney(status.expectedProfit)}</td>
                      <td className={`text-left ${getColorClass(status.roi)}`}>
                        {formatPercent(status.roi)}
                      </td>
                      <td className="text-left">{formatPercent(status.avgEV)}</td>
                    </tr>
                  ))}
      ...
      {/* Recent Bets Section */}
      <div>
        <h2 className="text-xl font-semibold mb-4 text-gray-800">All Bets</h2>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Understanding your complete betting history</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <h3 className="font-semibold text-gray-800 mb-2">What's Included</h3>
            <p className="text-gray-600">
              This page shows your complete betting history including all bet statuses: won, lost, pending, push, and void bets. 
              Expected value analysis is only available for bets with closing line value (CLV) data.
            </p>
          </div>

          <div>
            <h3 className="font-semibold text-gray-800 mb-2">Bet Status Definitions</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">Won</span>
                <span className="text-gray-600">Bet was successful</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs font-medium">Lost</span>
                <span className="text-gray-600">Bet was unsuccessful</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-medium">Pending</span>
                <span className="text-gray-600">Bet outcome not yet determined</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-gray-100 text-gray-800 rounded-full text-xs font-medium">Push</span>
                <span className="text-gray-600">Bet tied, stake returned</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-purple-100 text-purple-800 rounded-full text-xs font-medium">Void</span>
                <span className="text-gray-600">Bet cancelled, stake returned</span>
              </div>
            </div>
          </div>

          <div>
            <h3 className="font-semibold text-gray-800 mb-2">Expected Value Analysis</h3>
            <p className="text-gray-600">
              EV% and expected profit calculations are only shown for bets where closing line value data is available. 
              Bets without CLV data still contribute to overall profit/loss and win rate statistics.
            </p>
          </div>
        </CardContent>
      </Card>
